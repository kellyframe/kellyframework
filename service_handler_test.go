...
func uploadFileFunction(context *ServiceMethodContext, param *validatorEnabled) error {
...
func TestServiceHandlerServeHTTP(t *testing.T) {
	...
	h6, _ := NewServiceHandler(uploadFileFunction, nil, false, false)
	...
	emptyFunctionNormalUploadFile := httptest.NewRequest("POST", "/uploadFileFunction?a=1", bodyBuffer)
